    ...
    for (let i = 0; i < 6; i++) {
      for (let j = 0; j < 7; j++) {
        ...
        (this.params.mode === MODE_MONTH &&
          ...
          (this.params.mode === MODE_WEEK && //周模式下都可见
            this.drawItemListener &&
            this.drawItemListener($el, date));

        //监听 //TODO 此处可优化，每次调用show都会重新设置监听，其实没必要，但由于不重新设置，绑定的date就是旧值，因此可以传递日期坐标进行优化
        $el.on("click", event => {
          ((this.params.mode === MODE_MONTH &&
            ((this.params.outMonthShowable && this.params.outMonthClickable) ||
              (date.getMonth() === this.params.seedDate.getMonth() &&
                this.params.seedDate.getFullYear() === date.getFullYear()))) || //月模式
            this.params.mode === MODE_WEEK) && //周模式
            (this.onDateSelectedListeners &&
              (this.onDateSelectedListeners.forEach(listener => {
                listener &&
                  listener(new Date(date), new Date(this.params.selectDate));
              }) ||
                true) &&
